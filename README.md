...
Telegram‑бот для скачивания музыки из **Spotify**, распознавания треков по голосовым сообщениям (Shazam) и получения текста песен (Genius). Поддержка **YouTube** по ссылкам сейчас отключена (см. раздел «YouTube: как включить обратно»).
...
- (опционально) `YTDL_COOKIES` — cookies для YouTube (Netscape format); см. комментарии в `config.env.example`
