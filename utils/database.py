...
class db:
    ...
    async def get_user_language(user_id):
        result = await db.fetch_one('SELECT language FROM user_language WHERE user_id = ?', (user_id,))
    ...
    async def set_user_language(user_id, language):
        await db.execute_query('INSERT OR REPLACE INTO user_language (user_id, language) VALUES (?, ?)',
                               (user_id, language))
